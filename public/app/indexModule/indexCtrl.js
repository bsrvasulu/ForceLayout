...
(function (angular) {
    ...
    angular.module('mainApp').controller('indexCtrl', [
        ...
        function ($scope, $rootScope, $compile, indexService, $log, $timeout) {
                            ...
                            row.graph.nodes.forEach(function (n) {
                                n.labels.forEach(function (l){
                                    if('SME' === l){
                                        ...
                                        $scope.selectedNode.SMEs.push(SMEInfo);
                                    }
                                });
                            });
                        });
            ...
            $scope.UpdateSiteLayout = function (siteName) {
                $log.info('UpdateSiteLayout: siteName - ' + siteName);
                $log.info('$scope.hostInventoryCollection:' + JSON.stringify($scope.hostInventoryCollection));
                $scope.buildingRow.splice(0, $scope.buildingRow.length);

            };
            $scope.force = null;
            $scope.layoutRoot = null;
            var tcWhite = '#FFFFFF';
            $scope.nodes = {};
            $scope.links = [];
            ...
            $scope.clearNodes= function() {
                ...
                $scope.links = [];
                $scope.force.start();
                d3.timer($scope.force.resume);
                $scope.update();
            };

            $scope.drawForceLayout = function(){
                //,
                //root;
                ...
                $scope.layoutRoot = $scope.json;
                $scope.layoutRoot.fixed = true;
                $scope.layoutRoot.x = w / 2;
                $scope.layoutRoot.y = h / 4;

                var treeStructureObj = $scope.buildNodesAndLinks($scope.layoutRoot);

                $scope.nodes = treeStructureObj.nodes;
                $scope.links = treeStructureObj.links;
            ...
            $scope.update = function() {
                console.log('links: ' + JSON.stringify($scope.links));
                // Restart the force layout.
                $scope.force.nodes($scope.nodes)
                    .links($scope.links)
                    .gravity(0.05)
                    .charge(-2500)
                    .linkDistance(200)
                ...
                // Enter any new nodes.
                $scope.nodeEnter = node.enter().append('svg:g')
                    .attr('class', 'node')
                    .attr('transform', function(d) { return 'translate(' + d.x + ',' + d.y + ')'; })
                    .on('dblclick', $scope.click)
                    .on('click', $scope.updateNodeData)
                    .call($scope.force.drag);

                var nodeCircles2 = $scope.nodeEnter.append('svg:circle')
                ...
                // Append a circle
                var nodeCircles = $scope.nodeEnter.append('svg:circle')
                ...
                // Append images
                var images = $scope.nodeEnter.append('svg:rect')
                ...
                var textCodeCircle = $scope.nodeEnter.append('text')
                //.attr('class', 'nodetext')
                //.attr('dx', -15)
                ...
                function tick() {
                    ...
                    node.attr('transform', $scope.nodeTransform);
                }
             ...
             */
            $scope.nodeTransform =  function (d) {
            ...
            $scope.click = function(d) {
                ...
                $scope.layoutRoot.results[0].data.forEach(function (row) {
                    row.graph.nodes.forEach(function (n) {
            ...
            $scope.getSourceNode = function(n, id) {
                for (var i=0;i<n.length;i++) {
                    if (n[i].id == id){
            ...
            $scope.isLinkExist = function (allLinks, currLink){
                var bReturn = false;
            ...
            $scope.buildNodesAndLinks = function(neo4jJson){
                var nodes=[], links=[];
                ...
                neo4jJson.results[0].data.forEach(function (row) {
                    ...
                    row.graph.relationships.forEach(function (r) {
                        var link = {};
                        link.source = $scope.getSourceNode(nodes, r.startNode);
            ...
            $scope.getHostApplicationWithSME = function (siteName) {
                ...
                $scope.json = resultData.data;
                $scope.loadingHostInventory = false;
            ...
            $scope.UpdateHostApplicationLayout = function (siteName) {
                $log.info('UpdateHostApplicationLayout: siteName - ' + siteName);
                $scope.clearNodes();
                $scope.getHostApplication(siteName);
                $scope.layoutRoot = $scope.json;
                $scope.layoutRoot.fixed = true;
                $scope.layoutRoot.x = w / 2;
                $scope.layoutRoot.y = h / 4;
            ...
            $scope.getSMELinks = function (siteName) {
                $log.info('getSMELinks: siteName - ' + siteName);
                $scope.clearNodes();
                $scope.getHostApplicationWithSME(siteName);
            };
